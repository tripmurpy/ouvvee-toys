...
class CheckoutController extends Controller
{
        ...
        try {
            $order = DB::transaction(function () use ($request, $cart, $data): Order {
                ...
                foreach ($items as $item) {
                    ...
                    if (! $product || $product->status !== Product::STATUS_ACTIVE || $item->quantity > $product->stock) {
                ...
                $order->shipment()->create([
                    ...
                    'shipping_cost' => $shippingCost,
                    'shipment_status' => Shipment::STATUS_NOT_SHIPPED,
                ]);
    ...
    private function shippingCost(?string $shippingMethodId, int $weight): float
    {
        ...
        return $rate ? (float) $rate->base_cost + (ceil($weight / 1000) * (float) $rate->cost_per_kg) : -1;
    }

    private function nextOrderCode(): string
    {
        do {
            $code = sprintf('%s-%s-%s', config('app.order_code_prefix', 'OVV'), now()->format('ymd'), Str::upper(Str::random(5)));
